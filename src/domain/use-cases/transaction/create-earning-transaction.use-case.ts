...
> {
  ...
  protected async handle({
    ...
    try {
      ...
      const increasingUserBalance = async () => {
      ...
      if (transactedAt <= new Date()) {
        await increasingUserBalance();
      } else {
        await this.deps.jobSchedulingService.createUnique(
          increasingUserBalance,
          transactedAt,
          { id: earningTransaction.id.value, key: earningTransaction.id.value },
        );
      }

      if (recurrencePeriod) {
        await Promise.all([
          this.deps.earningTransactionRepository.createManyOfRecurrence(
            earningTransaction,
          ),
          this.deps.jobSchedulingService.createRepeatableByPeriod(
            increasingUserBalance,
            {
